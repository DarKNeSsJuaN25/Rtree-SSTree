...
class SSnode:
    ...
    def split(self, m):
        if self.leaf:
            ...
            data_left = self.data[:best_split_index]
            data_right = self.data[best_split_index:]
            newNode1 = SSnode(points=points_left, leaf=True ,data=data_left)
            newNode2 = SSnode(points=points_right, leaf=True ,data=data_right)
